// Italy regional DSU map — the signature interactive feature.
const { useState } = React;

function ItalyMap() {
  const regions = window.JWL.regions;
  const tierMeta = window.JWL.tierMeta;
          ...
          >
            {regions.map(r => (
              <g
                ...
                tabIndex={0}
                onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); setActive(r.id === active ? null : r.id); } }}
              >
                <polygon className={"tier-" + r.tier + "-f"} points={r.points} />
                <text className="region__lab" x={r.label[0]} y={r.label[1]}>{r.id}</text>
              </g>
            ))}
          </svg>
          ...
          <div className="legend">
            {Object.entries(tierMeta).map(([k, m]) => (
              <div className="legend__row" key={k}>
                <span className="legend__sw" style={{ background: "var(--tier-" + k + ")" }}></span>
                {m.label}
              </div>
            ))}
          </div>
        </div>
        <p className="map-hint">↳ Tap any region for stipend, income threshold &amp; your real odds</p>
      </div>
      ...
      <div className="rpanel">
        {!sel && (
          <div className="rpanel__empty">
            <p className="big">Pick a region.<br/>The numbers aren't the same.</p>
            <p>The DSU stipend, the income (ISEE) ceiling, and your actual odds of being funded all change depending on which region you study in.</p>
            <p className="arrow-note">← Start with the green ones if you want certainty.</p>
          </div>
        )}
        {sel && (
          <React.Fragment>
            <button className="rpanel__close" onClick={() => setActive(null)} aria-label="Close">×</button>
            <span className={"rpanel__tierbar " + sel.tier}>
              <span className="d" style={{ background: "var(--tier-" + sel.tier + ")" }}></span>
              {tierMeta[sel.tier].label}
            </span>
            <h3 className="rpanel__name">{sel.name}</h3>
            <p className="rpanel__sub">DSU regional scholarship · illustrative figures</p>
